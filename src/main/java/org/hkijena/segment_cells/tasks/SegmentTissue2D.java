...
public class SegmentTissue2D extends DAGTask {
    ...
    public SegmentTissue2D(Integer tid, SampleDataInterface sampleDataInterface, int planeZIndex) {
    ...
    @Override
    public void work() {
        System.out.println("Running SegmentTissue2D on " + getSampleDataInterface().getInputData().toString() + " z=" + planeZIndex);
        ...
        final double xsize = getSampleDataInterface().getVoxelSizeXY();
        ...
        getSampleDataInterface().addTissuePixelCount(Filters.countNonZero(mask));

        getSampleDataInterface().getTissueOutputData().setPlane(planeZIndex, mask);
